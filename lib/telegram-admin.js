...
function mainMenu() {
  return new InlineKeyboard()
    ...
    .text('🍜 Quản lý món', 'menu_items').row()
    .text('📂 Quản lý danh mục', 'menu_categories').row()
    .text('🖼 Quản lý banner', 'menu_banners').row()
    .text('📦 Xem đơn hàng', 'view_orders').row()
    .text('⚙️ Cài đặt', 'menu_settings').row()
    .text('💰 Doanh thu', 'menu_revenue');
}

function backButton(target) {
  return new InlineKeyboard().text('← Quay lại', target);
}
...
function startAdminBot(token) {
  ...
  bot.command('cancel', async (ctx) => {
    clearSession(ctx.chat.id);
    await ctx.reply('Đã hủy thao tác hiện tại.', { reply_markup: mainMenu() });
  });
  ...
  bot.callbackQuery('menu_items', async (ctx) => {
    ...
    keyboard.text('➕ Thêm món mới', 'add_item').row();
    if (items.length > 0) {
      keyboard.text('📋 Danh sách món', 'list_items').row();
    }
    keyboard.text('← Quay lại', 'start');
  ...
  bot.callbackQuery('list_items', async (ctx) => {
    ...
    keyboard.text('← Quay lại', 'menu_items');
  ...
  bot.callbackQuery('list_categories', async (ctx) => {
    ...
    keyboard.text('← Quay lại', 'menu_categories');
  ...
  bot.callbackQuery('list_banners', async (ctx) => {
    ...
    keyboard.text('← Quay lại', 'menu_banners');
  ...
  bot.callbackQuery(/^edit_banner_(\d+)$/, async (ctx) => {
    ...
    const banner = db.getDb().prepare('SELECT * FROM banners WHERE id = ?').get(bannerId);
  ...
  bot.callbackQuery(/^orders_(.+)$/, async (ctx) => {
    ...
    keyboard.text('← Quay lại', 'view_orders');
  ...
  bot.callbackQuery(/^pay_order_(\d+)$/, async (ctx) => {
    ...
    if (order && order.status !== 'cancelled' && order.status !== 'paid') {
      ...
      // Refresh
      const keyboard = new InlineKeyboard().text('← Quay lại', 'view_orders');
      await ctx.editMessageText(
